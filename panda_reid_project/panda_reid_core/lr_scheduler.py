...
def build_scheduler(config, optimizer, n_iter_per_epoch):
    ...
    # Guard short runs (e.g. EPOCHS=1) where warmup could consume all steps.
    safe_warmup_steps = min(warmup_steps, max(0, total_steps - 1))
    if config.TRAIN.LR_SCHEDULER.WARMUP_PREFIX:
        cosine_steps = max(1, total_steps - safe_warmup_steps)
    else:
    ...
    if config.TRAIN.LR_SCHEDULER.NAME == 'cosine':
        try:
            # Try with t_mul parameter (older timm versions)
            lr_scheduler = CosineLRScheduler(
                ...
                warmup_t=safe_warmup_steps,
                cycle_limit=1,
                t_in_epochs=False,
                warmup_prefix=config.TRAIN.LR_SCHEDULER.WARMUP_PREFIX,
            )
        except TypeError:
            # Try without t_mul parameter (newer timm versions)
            lr_scheduler = CosineLRScheduler(
                optimizer,
                t_initial=cosine_steps,
                lr_min=config.TRAIN.MIN_LR,
